...
const ConfirmInformation = () => {
    const {rideInfo, vehiclesDetails} = useSelector((state) => ({
            ...
            <Box sx={{marginBottom: 3}}>
                <Grid container spacing={2}>
                    <Grid xs={6}>
                        ...
                        <p>Pickup Time: {formatDateString(rideInfo.pickupDate)} {formatDateString(rideInfo.pickupTime, 'HH:mm')}</p>
                    ...
                    <Grid xs={6}>
                        <Typography variant='h5'>Vehicles Information</Typography>
                        <p>Name: {vehiclesDetails.firstName} {vehiclesDetails.lastName}</p>
                        <p>Email: {vehiclesDetails.email}</p>
                        <p>Phone Number: {vehiclesDetails.phon}</p>
                        <p>Vehicle Types: {vehiclesDetails.vehicleTypes.join()}</p>
                        <p>Special Requests: {vehiclesDetails.specialRequest}</p>
                    </Grid>
                </Grid>
