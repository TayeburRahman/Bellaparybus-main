...
exports.register = (email, password, firstName, lastName, phone) => { 
  return new Promise(async (resolve, reject) => {
    ...
    const user_exist = await db.query(`SELECT * FROM ${userTable} WHERE email_lowercased = '${email_lowercased}'`);

    // user already exists
    if (user_exist.length > 0) {
      reject({ status: 409, message: 'This email address is already used. Try a different email address!'});
      return;
    }
    // register a new user
    bcrypt.hash(password, saltRounds, function(err, hash) {
      ...
      crypto.randomBytes(32, async function(ex, buf) {
        ...
        var date = new Date();
        var token_expired = Date.now() + config.email_confirm_token_expiration;
        var email_confirm_token_expiration = new Date(token_expired);
        let phoneJSON = `{"mobile": ${phone}}`;

        const rows = await db.query(`INSERT INTO ${userTable}(email, email_lowercased, password, email_confirm_token, created_at, updated_at, email_confirm_token_expired, first_name, last_name, phone) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [email, email_lowercased, hash, email_confirmed_token, date, date, email_confirm_token_expiration, firstName, lastName, phoneJSON]);
 ...
 */
 exports.email_verification = (userId, token) => {
  return new Promise(async (resolve, reject) => {
    ...
    const user_exist = await db.query(`SELECT email_confirm_token FROM ${userTable} WHERE id = '${userId}'`);
    ...
    if(token === email_token) {
      ...
      const update_active_status = await db.query(`UPDATE ${userTable} SET active_status = 1, email_confirm_token='${randomToken}' WHERE id = '${userId}'`);
      const res = await db.query(`SELECT * FROM ${userTable} WHERE id = '${userId}'`);
      const user = res[0];
...
exports.forgotPassword = (email) => {
  return new Promise(async (resolve, reject) => {
    const email_lowercased = email.toLowerCase();
    const user_exist = await db.query(`SELECT id FROM ${userTable} WHERE email_lowercased = '${email_lowercased}'`);
    ...
    if(user_exist.length === 0) {
      ...
      crypto.randomBytes(32, async function(ex, buf) {
        ...
        var password_reset_token_expired = new Date(token_expired).toISOString().slice(0, 19).replace('T', ' ');;
        const setResetToken = await db.query(`UPDATE ${userTable} SET password_reset_token = '${password_reset_token}', password_reset_token_expired='${password_reset_token_expired}' WHERE id = '${id}'`);
 ...
 */
 exports.setForgotPassword = (userId, token, password) => {
  return new Promise(async (resolve, reject) => {
    const user_exist = await db.query(`SELECT password_reset_token FROM ${userTable} WHERE id = '${userId}'`);
    ...
    if(token === password_reset_token) {
      bcrypt.hash(password, saltRounds, async function(err, hash) {
        ...
        const randomToken = makeid(10);
        const update_user_password = await db.query(`UPDATE ${userTable} SET password_reset_token = '${randomToken}', password='${hash}' WHERE id = '${userId}'`);
 ...
 exports.deleteAccount = (request) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (data) {
      ...
      const deletedAccount = await db.query(`DELETE from ${userTable} where id = ${userId}`);
