...
exports.login = (email, password) => { 
  return new Promise(async (resolve, reject) => {
    ...
    const admin = await db.query(`SELECT * FROM ${adminTable} WHERE email_lowercased = '${email_lowercased}'`);
...
exports.requestForgotPassword = (email) => {
  return new Promise(async (resolve, reject) => {
    ...
    crypto.randomBytes(32, async function(ex, buf) {
      ...
      var password_reset_expiration = new Date(token_expired).toISOString().slice(0, 19).replace('T', ' ');
      const rows = await db.query(`UPDATE ${adminTable} SET password_reset_token = '${password_reset_token}', password_reset_token_expired = '${password_reset_expiration}' WHERE email_lowercased = '${email_lowercased}'`);
